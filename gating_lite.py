...
for trial in range(len(trial_order)):  # trial_order:
    ...
    beep = sound.Sound('lijffijt_40ms_90dB_1000Hz_4ms_rise_fall.wav')
    # beep2 = sound.Sound('beep_20ms.wav', volume=0.4)
    # clik = sound.Sound('click_20ms.wav', volume=1.0)
    # go for trial
    sound_trial(sound_type=beep if trial_type == 'beep' else None, trial_type_name=trial_type, trial_number=trial, n_trials=num_trials,
                trial_initial_silence=2.0, trial_final_silence=random.uniform(6.0, 8.0))
